...
function majDateAnalyse(nodeid) {
	var date = $("*:has(>metadata[semantictag*='date-analyse'])",$(UICom.structure.ui[nodeid].node).parent())[0];
	var dateid = $(date).attr("id");
	const options = {year: 'numeric', month: 'numeric', day: 'numeric'};
	const today = new Date();
	UICom.structure.ui[nodeid].label_node[LANGCODE].text(today.toLocaleString(undefined,options));
