...
function specificDisplayPortfolios(type){
	if (USER.other=="enseignant" || USER.other=="responsable") {
		if (type==null)
			type = 'card';
		...
		for (var i=0;i<portfolios_list.length;i++){
			//--------------------------
			if (portfolios_list[i].visible && $(portfolios_list[i].code_node).text().indexOf('portfolio-etu')<0 && $(portfolios_list[i].code_node).text().indexOf('portfolio-alternance')<0 ) {
	...
	} else if (USER.other=="tuteur"){
		for (var i=0;i<portfolios_list.length;i++){
			//--------------------------
			if (portfolios_list[i].visible && $(portfolios_list[i].code_node).text().indexOf('portfolio-tuteur')>-1) {
				display_main_page(portfolios_list[i].id);
				break;
			}
		}		
...
function getCohorte(type,portfoliocode) {  // type = courantes|anciennes - portfoliocode: partie du code
	...
	const curday = parseInt(today.getDate());
	const searchvalue = "-20";
	
	console.log("Type : " + type);
	console.log("portfoliocode : " + portfoliocode);
