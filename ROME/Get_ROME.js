...
//		let portfoliocode = cleanCode(replaceVariable(this.query_portfolio));
		...
		//------------
		var self = this;
		if (cachable && g_Get_ROME_caches[portfoliocode+semtag]!=undefined && g_Get_ROME_caches[portfoliocode+semtag]!="")
			self.parse(destid,type,langcode,g_Get_ROME_caches[portfoliocode+semtag],disabled,srce,resettable,target,semtag,null,portfoliocode,semtag2,cachable);
		else {
			$.ajax({
				async:false,
				type : "GET",
				...
				success : function(data) {
					if (cachable)
						g_Get_ROME_caches[portfoliocode+semtag] = data;
...
//==================================
UIFactory["Get_ROME"].getChildren = function(dest,self,langcode,srce,target,portfoliocode,semtag,semtag_parent,code,cachable,tabadded)
//==================================
{
	var semtag2 = "";
	if (semtag.indexOf('+')>-1) {
	...
	//------------
	if (cachable && g_Get_ROME_caches[portfoliocode+semtag+semtag_parent+code]!=undefined && g_Get_ROME_caches[portfoliocode+semtag+semtag_parent+code]!="")
		UIFactory.Get_ROME.parseChildren(dest,self,g_Get_ROME_caches[portfoliocode+semtag+semtag_parent+code],langcode,srce,target,portfoliocode,semtag,semtag_parent,code,semtag2,cachable,tabadded)
	else {
		$.ajax({
			async:false,
			type : "GET",
			...
			success : function(data) {
				if (cachable)
					g_Get_ROME_caches[portfoliocode+semtag+semtag_parent+code] = data;
				UIFactory.Get_ROME.parseChildren(dest,self,data,langcode,srce,target,portfoliocode,semtag,semtag_parent,code,semtag2,cachable,tabadded)
			}
		});
	}
	//------------
...
//==================================
UIFactory["Get_ROME"].parseChildren = function(dest,self,data,langcode,srce,target,portfoliocode,semtag,semtag_parent,code,semtag2,cachable,tabadded)
//==================================
{
	//-----Node ordering-------------------------------------------------------
	var nodes = $("node",data);
	...
	for ( var i = 0; i < newTableau2.length; ++i) {
		...
		if (semtag2!="") {
			...
			UIFactory.Get_ROME.getChildren(dest,self,langcode,srce,target,portfoliocode,semtag2,semtag_parent,code,cachable);
		}
	}
}
...
//==================================
UIFactory["Get_ROME"].addSimple = function(parentid,targetid,multiple_tags)
//==================================
{
	//------------------------------
	...
	var databack = true;
	var callback = UIFactory.Get_ROME.updateaddedpart;
...
//==================================
UIFactory["Get_ROME"].addMultiple = function(parentid,targetid,multiple_tags,updated_semtag,fctjs)
//==================================
{
	if (fctjs==null)
		fctjs = "";
...
//==================================
UIFactory["Get_ROME"].updateaddedpart = function(data,get_resource_semtag,selected_item,last,parentid,fct)
//==================================
{
	var partid = data;
	var value = $(selected_item).attr('value');
	...
	var xml = "<asmResource xsi_type='Get_ROME'>";
	xml += "<code>"+code+"</code>";
...
//==================================
UIFactory["Get_ROME"].importMultiple = function(parentid,targetid,srce,fctjs)
//==================================
{
	if (fctjs==null)
		fctjs = "";
...
//==================================
{
	...
	var js2 = "UIFactory.Get_ROME.addSimple('"+parentid+"','"+targetid+"','"+replaceVariable(partcode)+","+get_resource_semtag+"')";
	...
	var getResource = new UIFactory["Get_ROME"](UICom.structure.ui[parentid].node,"xsi_type='nodeRes'");
...
//==================================
{
	...
	var js1 = "javascript:$('#edit-window').modal('hide')";
	var js2 = "UIFactory.Get_ROME.addMultiple('"+parentid+"','"+targetid+"','"+replaceVariable(partcode)+","+get_resource_semtag+","+fct+"')";
	var footer = "<button class='btn' onclick=\""+js2+";\">"+karutaStr[LANG]['Add']+"</button> <button class='btn' onclick=\""+js1+";\">"+karutaStr[LANG]['Close']+"</button>";
	$("#edit-window-footer").html(footer);
	$("#edit-window-title").html(title.replaceAll("##apos##","'"));
	var html = "<div id='get-resource-node'></div>";
...
{
	var js1 = "javascript:$('#edit-window').modal('hide')";
	var js2 = "UIFactory.Get_ROME.importMultiple('"+parentid+"','"+targetid+"','"+replaceVariable(partcode)+"')";
	var footer = "<button class='btn' onclick=\""+js2+";\">"+karutaStr[LANG]['Add']+"</button> <button class='btn' onclick=\""+js1+";\">"+karutaStr[LANG]['Close']+"</button>";
	$("#edit-window-footer").html(footer);
	$("#edit-window-title").html(title.replaceAll("##apos##","'"));
	var html = "<div id='get-resource-node'></div>";
	...
	for (let i=0;i<actions.length;i++) {
		...
		if (actions[i].type=="import_component") {
			...
			for (let j=0;j<targets.length;j++) {
				if (targetid=="")
					targetid = targets[j];
				js2 += "UIFactory.Get_ROME.addMultiple('"+actions[i].parentid+"','"+targets[j]+"','"+replaceVariable(actions[i].foliocode+"."+actions[i].semtag)+"','"+actions[i].updatedtag+"','"+fctjs+"');";
			}
			...
			for (let j=0;j<targets.length;j++) {
				if (targetid=="")
					targetid = targets[j];
				js2 += "UIFactory.Get_ROME.importMultiple('"+actions[i].parentid+"','"+targets[j]+"','"+replaceVariable(actions[i].foliocode+"','"+actions[i].semtag)+"','"+fctjs+"');";
			}
			...
			for (let j=0;j<targets.length;j++) {
				if (targetid=="")
					targetid = targets[j];
				js2 += "UIFactory.Get_ROME.importMultiple('"+actions[i].parentid+"','"+targets[j]+"','"+replaceVariable(actions[i].foliocode)+"','"+fctjs+"');";
			}
