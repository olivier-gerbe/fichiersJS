...
plugin_resources[plugin_resources.length] = {location:'karuta.configuration',tag:'ROME',label:'ROME'};
...
//==================================
{
	...
	if (this.clause=="xsi_type='ROME'")
		this.multilingual = ($("metadata",node).attr('multilingual-resource')=='Y') ? true : false;
	else // asmUnitStructure - ROME
...
//==================================
UIFactory["ROME"].prototype.getAttributes = function(type,langcode)
//==================================
{
	var result = {};
	//---------------------
...
//==================================
UIFactory["ROME"].prototype.getCode = function(dest)
//==================================
{
	if (dest!=null) {
...
//==================================
UIFactory["ROME"].prototype.getValue = function(dest)
//==================================
{
	if (dest!=null) {
...
//==================================
UIFactory["ROME"].prototype.getView = function(dest,type,langcode,indashboard)
//==================================
{
	//---------------------
	if (indashboard==null)
...
//==================================
UIFactory["ROME"].prototype.displayView = function(dest,type,langcode)
//==================================
{
	//---------------------
	if (langcode==null)
...
/// Editor
//==================================
UIFactory["ROME"].update = function(selected_item,itself,langcode,type)
//==================================
{
...
//==================================
UIFactory["ROME"].prototype.displayEditor = function(destid,type,langcode,disabled,cachable,resettable)
//==================================
{
	...
	if (queryattr_value!=undefined && queryattr_value!='') {
		...
		var self = this;
		if (cachable && g_ROME_caches[queryattr_value]!=undefined && g_ROME_caches[queryattr_value]!="")
			UIFactory["ROME"].parse(destid,type,langcode,g_ROME_caches[queryattr_value],self,disabled,srce,resettable,target,semtag,multiple_tags);
		else {
			$.ajax({
				type : "GET",
				dataType : "json",
				...
				success : function(data) {
					if (cachable)
						g_ROME_caches[queryattr_value] = data;
					UIFactory["ROME"].parse(destid,type,langcode,data,self,disabled,srce,resettable,target,semtag,multiple_tags);
				}
			});
		}
	}
...
//==================================
UIFactory["ROME"].prototype.save = function()
//==================================
{
	if (this.clause=="xsi_type='ROME'") {
...
//==================================
UIFactory["ROME"].prototype.refresh = function()
//==================================
{
...
//==================================
UIFactory["ROME"].importMultiple = function(parentid,srce)
//==================================
{
	$.ajaxSetup({async: false});
	var inputs = $("input[name='multiple_"+parentid+"']").filter(':checked');
...
//==================================
UIFactory["ROME"].updateaddedpart = function(data,ROME_semtag,selected_item,last)
//==================================
{
	var partid = data;
	var value = $(selected_item).attr('value');
	var code = $(selected_item).attr('code');
	var xml = "<asmResource xsi_type='ROME'>";
	xml += "<code>"+code+"</code>";
	...
	$.ajax({
		...
		last : last,
		success : function(data) {
//			var nodeid = $("asmContext:has(metadata[semantictag='"+ROME_semtag+"'])",data).attr('id');
			var node = $("*:has(metadata[semantictag='"+ROME_semtag+"'])",data);
			if (node.length==0)
...
//==================================
function get_rome_multiple(parentid,title,query,partcode,ROME_semtag)
//==================================
{
	var langcode = LANGCODE;
	//---------------------
	var js1 = "javascript:$('#edit-window').modal('hide')";
	var js2 = "UIFactory.ROME.addMultiple('"+parentid+"','"+partcode+","+ROME_semtag+"')";
	...
	var getResource = new UIFactory["ROME"](UICom.structure["ui"][parentid].node,"xsi_type='nodeRes'");
	getResource.multiple = query+"/"+partcode+","+ROME_semtag;
...
//==================================
function import_rome_multiple(parentid,title,query,partcode,ROME_semtag)
//==================================
{
	var langcode = LANGCODE;
	//---------------------
	var js1 = "javascript:$('#edit-window').modal('hide')";
	var js2 = "UIFactory.ROME.importMultiple('"+parentid+"','"+partcode+"')";
	var footer = "<button class='btn' onclick=\""+js2+";\">"+karutaStr[LANG]['Add']+"</button> <button class='btn' onclick=\""+js1+";\">"+karutaStr[LANG]['Close']+"</button>";
	$("#edit-window-footer").html(footer);
	$("#edit-window-title").html(title);
	var html = "<div id='get-resource-node'></div>";
...
//==================================
UIFactory["ROME"].parse = function(destid,type,langcode,data,self,disabled,srce,resettable,target,semtag,multiple_tags) {
//==================================
	//---------------------
	if (langcode==null)
		langcode = LANGCODE;
	...
	if (type=='select') {
		...
		for ( var i = 0; i < newTableau1.length; i++) {
			...
			$(select_item_a).click(function (ev){
				...
				UIFactory["ROME"].update(this,self,langcode);
				//--------------------------------
			});
	...
	if (type=='completion') {
		...
		$(form).append($(btn_group));
		$("#"+destid).append(form);
		var onupdate = "UIFactory.ROME.update(inp,self)";
...
console.log("ROME plugin loaded.");
