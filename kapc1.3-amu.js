		...
		if (type==type1) {
			...
			if (date1!=null || date2!=null) {
				...
				if (date1!=null && date2!=null && date1<date && date<date2+86400000) {
						if (tab1.indexOf(nodeid+"/"+pageid+"/"+portfolioid)<0)
							tab1.push(nodeid+"/"+pageid+"/"+portfolioid);
			...
			} else if (tab1.indexOf(nodeid+"/"+pageid+"/"+portfolioid)<0)
				tab1.push(nodeid+"/"+pageid+"/"+portfolioid);
			}
	}
	if (type2!=null && type2!=""){
...
//		const search2 = $("vector",searchVector(enseignantid,type2));
		for (let i=0;i<search1.length;i++){
			...
			if (type==type2) {
				...
				let pageid = $("a4",search1[i]).text();
				let portfolioid = $("a5",search1[i]).text();
				const indx = tab1.indexOf(nodeid+"/"+pageid+"/"+portfolioid);
				if (indx>-1)
					tab1.splice(indx,1);
			}
		}
	}
	// on garde celles qui n'ont pas été évaluées par un autre professuer
	const searchType2 = $("vector",searchVector(null,type2));
	let types2 = [];
	for (let i=0;i<searchType2.length;i++) {
		let pageid = $("a4",searchType2[i]).text();
		types2.push(pageid);
	}
	for (let i=0;i<tab1.length;i++) {
		const elts = tab1[i].split("/");
		let pageid = elts[1];
		const indx = types2.indexOf(pageid);
		if (indx<0)
			tab2.push(tab1[i]);
	}	
	//---------------
...
function numberVectorKAPC(enseignantid,type1,type2,date1,date2) {
	...
	let tab2 = [];
	let tab3 = [];
	const search1 = $("vector",searchVector(enseignantid));
	// on ajoute tous les uuids qui ont type1
	for (let i=0;i<search1.length;i++){
		let type= $("a2",search1[i]).text();
		if (type==type1) {
...
//		const search2 = $("vector",searchVector(enseignantid,type2));
	...
	// on garde celles qui n'on pas été évaluées par un autre professuer
	const searchType2 = $("vector",searchVector(null,type2));
	let types2 = [];
	for (let i=0;i<searchType2.length;i++) {
		let pageid = $("a4",searchType2[i]).text();
...
//============================================================
//============================================================
function rechercheetudiantid(destid,enseignantid,etudiantid) {
	enseignantid = replaceVariable(enseignantid);
	etudiantid = replaceVariable(etudiantid);
	let found = false;
	let search = $("vector",searchVector(enseignantid));
	let result = [];
	for (let i=0; i<search.length;i++) {
		const a6 = $("a6",search[i]).text();
		if (a6.indexOf(etudiantid)>-1) {
			found = true;
	...
	if (!found)
		$('#'+destid).append("<div>aucun résultat</div>");
}
...
function displayEvaluation(destid,date,a1,a2,a3,a4,a5,a6,a7,a8,a9,a10) {
	...
	const date_demande = new Date(parseInt(a7));
	html += "<td>"+ date_demande.toLocaleString()+"</td>";
	html += "<td>"+a8+"<span class='button fas fa-binoculars' onclick=\"previewPage('"+a4+"',100,'previewURL',null,true)\" data-title='Aperçu' data-toggle='tooltip' data-placement='bottom' ></span></td>";
//	html += "<td>"+a10+"</td>";
//	html += "<td>"+a9+"</td>";
	html += "</tr>";
	$("#"+destid).append(html);
